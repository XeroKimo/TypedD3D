...
namespace TypedD3D::Helpers::D3D12
{
    ...
    Microsoft::WRL::ComPtr<Device> CreateDevice(D3D_FEATURE_LEVEL minFeatureLevel, IDXGIAdapter* optAdapter = nullptr)
    {
    ...
    Microsoft::WRL::ComPtr<CommandList> CreateCommandList(ID3D12Device& device, D3D12_COMMAND_LIST_TYPE type, ID3D12CommandAllocator& allocator, UINT nodeMask = 0, ID3D12PipelineState* optInitialPipeline = nullptr)
    {
    ...
    template<class CommandList = ID3D12GraphicsCommandList>
    Microsoft::WRL::ComPtr<CommandList> CreateCommandList(ID3D12Device4& device, D3D12_COMMAND_LIST_TYPE type, D3D12_COMMAND_LIST_FLAGS flags, UINT nodeMask = 0)
    {
    ...
    inline Microsoft::WRL::ComPtr<ID3D12CommandAllocator> CreateCommandAllocator(ID3D12Device& device, D3D12_COMMAND_LIST_TYPE type)
    {
        return Helpers::COM::IIDToObjectForwardFunction<ID3D12CommandAllocator>(&ID3D12Device::CreateCommandAllocator, device, type);
    }

    inline Microsoft::WRL::ComPtr<ID3D12PipelineState> CreateGraphicsPipelineState(ID3D12Device& device, const D3D12_GRAPHICS_PIPELINE_STATE_DESC& desc)
    {
        return Helpers::COM::IIDToObjectForwardFunction<ID3D12PipelineState>(&ID3D12Device::CreateGraphicsPipelineState, device, &desc);
    }

    inline Microsoft::WRL::ComPtr<ID3D12PipelineState> CreateComputePipelineState(ID3D12Device& device, const D3D12_COMPUTE_PIPELINE_STATE_DESC& desc)
    {
        return Helpers::COM::IIDToObjectForwardFunction<ID3D12PipelineState>(&ID3D12Device::CreateComputePipelineState, device, &desc);
    }

    inline Microsoft::WRL::ComPtr<ID3D12CommandQueue> CreateCommandQueue(ID3D12Device& device, const D3D12_COMMAND_QUEUE_DESC& desc)
    {
        return Helpers::COM::IIDToObjectForwardFunction<ID3D12CommandQueue>(&ID3D12Device::CreateCommandQueue, device, &desc);
    }

    inline Microsoft::WRL::ComPtr<ID3D12Fence> CreateFence(ID3D12Device& device, D3D12_FENCE_FLAGS flags, UINT64 initialValue = 0)
    {
        return Helpers::COM::IIDToObjectForwardFunction<ID3D12Fence>(&ID3D12Device::CreateFence, device, initialValue, flags);
    }

    inline Microsoft::WRL::ComPtr<ID3D12DescriptorHeap> CreateDescriptorHeap(ID3D12Device& device, const D3D12_DESCRIPTOR_HEAP_DESC& descriptorHeapDesc)
    {
    ...
    inline Microsoft::WRL::ComPtr<ID3D12RootSignature> CreateRootSignature(ID3D12Device& device, UINT nodeMask, const void* pBlobWithRootSignature, SIZE_T blobLengthInBytes)
    {
    ...
    inline Microsoft::WRL::ComPtr<ID3D12QueryHeap> CreateQueryHeap(ID3D12Device& device, const D3D12_QUERY_HEAP_DESC& desc)
    {
        return Helpers::COM::IIDToObjectForwardFunction<ID3D12QueryHeap>(&ID3D12Device::CreateQueryHeap, device, &desc);
    }

    inline Microsoft::WRL::ComPtr<ID3D12CommandSignature> CreateCommandSignature(ID3D12Device& device,
    ...
    inline Microsoft::WRL::ComPtr<ID3D12Resource> CreateCommittedResource(ID3D12Device& device,
    ...
    inline Microsoft::WRL::ComPtr<ID3D12Heap> CreateHeap(
        ID3D12Device& device,
    ...
    inline Microsoft::WRL::ComPtr<ID3D12Resource> CreatePlacedResource(ID3D12Device& device,
        ID3D12Heap& pHeap,
        UINT64 HeapOffset,
        const D3D12_RESOURCE_DESC& pDesc,
    ...
    inline Microsoft::WRL::ComPtr<ID3D12Resource> CreateReservedResource(ID3D12Device& device,
        const D3D12_RESOURCE_DESC& pDesc,
        D3D12_RESOURCE_STATES InitialState,
        const D3D12_CLEAR_VALUE* optOptimizedClearValue)
    {
    ...
    Microsoft::WRL::ComPtr<DebugTy> GetDebugInterface()
    {
        return Helpers::COM::IIDToObjectForwardFunction<DebugTy>(&D3D12GetDebugInterface);
